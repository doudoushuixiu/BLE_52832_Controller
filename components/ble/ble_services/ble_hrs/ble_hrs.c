...
uint32_t ble_hrs_init(ble_hrs_t * p_hrs, const ble_hrs_init_t * p_hrs_init)
{
    ...
    // Initialize service structure
	  p_hrs->evt_handler                 = p_hrs_init->evt_handler; //main中声明为空
	  p_hrs->data_handler                = p_hrs_init->data_handler;  //数据接收
    p_hrs->conn_handle                 = BLE_CONN_HANDLE_INVALID;

    // Add service
    BLE_UUID_BLE_ASSIGN(ble_uuid, BLE_UUID_HEART_RATE_SERVICE);   //赋值给uuid 0x180D

    err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY,  //添加servers
                                        &ble_uuid,
